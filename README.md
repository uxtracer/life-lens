...
进入 `http://127.0.0.1:7878`，默认是**配置 tab**，有 5 张卡片(按推荐顺序):
...
4. **📁 数据源** —— 添加照片目录。可以是本地任意含图片的路径，macOS 用户也可以直接添加 Apple Photos 库(`~/Pictures/Photos Library.photoslibrary`)。
5. **💾 存储** —— 查看数据库大小、缓存大小、备份目录。

依赖配齐后，顶部 banner 就会消失，可以开始用了。

### 之后的流程

1. **面孔 tab** —— 强烈建议先添加你常出现的家人朋友的种子照片(每人 3-5 张正脸，不同角度)。这样扫描出来的照片描述里直接带姓名，事后不用重跑。
2. **扫描 tab** —— 选数据源，点开始。按拍照时间正序处理。中途合盖、关电脑都不影响，下次自动续传。
3. **浏览 tab** —— 看扫描结果，点开看大图和完整识别信息。
4. **问相册 tab** —— 自然中文提问，模型会自动选合适的查询方式找照片给你看。
